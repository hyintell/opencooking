# Recipe planning
from opencooking.recipe_planner.stripsworld import STRIPSWorld
import opencooking.recipe_planner.utils as recipe
from opencooking.recipe_planner.recipe import *

# Delegation planning
# from delegation_planner.bayesian_delegator import BayesianDelegator

# Navigation planning
import opencooking.navigation_planner.utils as nav_utils

# Other core modules
import os
from opencooking.utils.interact import interact
from opencooking.utils.world import World
from opencooking.utils.core import *
from opencooking.utils.agent import SimAgent
from opencooking.misc.game.gameimage import GameImage
from opencooking.utils.agent import COLORS
from opencooking.utils.utils import MAP_DIR
...
class OvercookedEnvironment(gym.Env):
    ...
    def __init__(self, num_agents, level, arglist):
        ...
        self.t = 0
        self.filename = ""
        if hasattr(arglist, "model1") or hasattr(arglist, "model2"):
            self.set_filename()
    ...
    def __copy__(self):
        new_env = OvercookedEnvironment(self.num_agents, self.level, self.arglist)
    ...
    def set_filename(self):
        self.filename = "{}_agents{}_seed{}".format(self.level,\
            self.num_agents, self.arglist.seed)

    def load_level(self, level, num_agents):
        x = 0
        y = 0
        map_path = os.path.join(MAP_DIR, './{}.txt'.format(level))
        # with open(r'../utils/levels/{}.txt'.format(level), 'r') as file:
